...
public class BookControllerTest {
    ...
    @Test
    void shouldGetBooksPublishedAfter() {
        LocalDate date = LocalDate.of(2021, 1, 1);

        List<Book> expectedBooks = List.of(
                new Book(1L, "title-1", "author-1", LocalDate.of(2021, 2, 3)),
                new Book(2L, "title-2", "author-2", LocalDate.of(2021, 2, 3))
        );

        when(this.bookService.findPublishedAfter(date)).thenReturn(expectedBooks);

        this.graphQlTester
                .documentName("findBooksPublishedAfter")
                .variable("date", date.toString())
                .execute()
                .path("findBooksPublishedAfter")
                .matchesJson("""
                [
                    {
                        "id": 1,
