...
export const fetchBooksPublishedAfter = async (date: string): Promise<Book[]> => {
  const response = await fetch(GRAPHQL_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      query: `query($date: String!) { findBooksPublishedAfter(date: $date) {
            id
            title
            author
            publishedDate
  ...
  const { data } = await response.json();
  return data.findBooksPublishedAfter;
};
